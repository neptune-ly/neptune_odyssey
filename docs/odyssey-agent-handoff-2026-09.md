...
9. **Live Figma audit snapshot:** `contracts/odyssey-figma-audit.json`
10. **Implementation log:** `docs/odyssey-implementation-log-2026-09-18.md`
...
- 68 web concepts have a verified exact public KMP symbol and exact Kotlin source file.
- 21 web concepts deliberately make no one-to-one KMP symbol claim.
- React and Vue each expose 89 generated typed wrappers; Svelte consumes all 89 custom elements natively.
...
The component drift checker fails if web registration and the contract diverge, if audited Figma node/page metadata drifts, if an exact source or verified KMP symbol disappears, if wrapper symbols diverge, or if a host API pretends to have a visual node.
...
React/Vue wrapper exports are generated by `tools/generate-framework-wrappers.mjs`. Never hand-maintain the export list. Run `pnpm wrappers:generate` after changing the registered surface and commit the generated result. Svelte remains native to web custom elements.
...
### KMP
KMP is promoted and guarded by goldens, no-literal checks, multi-target builds and render sweeps. Exact symbol-level mapping must be verified in source before claiming a Figma component is KMP-canonical. The machine contract uses `verified-native-symbol` only when the exact public declaration is present in the recorded Kotlin file; it does not infer equivalents from similar names or gallery usage.

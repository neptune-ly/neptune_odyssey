...
class NeptuneBreadcrumbs extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final shape = Theme.of(context).extension<NptShape>()!;
    final text = Theme.of(context).textTheme;
    // ONE CHEVRON, MIRRORED ONCE. `chevron_right_rounded` carries
    // `matchTextDirection: true`, so `Icon` already flips it under RTL; picking
    // the left glyph here as well mirrored it twice and the separator pointed
    // back up the trail in every RTL locale.
...
class NeptunePagination extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        _ArrowButton(
          // Both glyphs carry `matchTextDirection: true`, so `Icon` mirrors

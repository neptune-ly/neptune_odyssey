...
> - **React Native** (`@neptune.fintech/react-native-ui`) — provider + hook + themed native components reading the resolved theme from `@neptune.fintech/tokens` `buildTheme()` (no web custom elements). Both add no color math, inheriting the determinism contract from `tokens`.

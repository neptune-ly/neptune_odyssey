...
function emitKotlinFixtures() {
  ...
  L.push('// and the reference brandprints + the synthetic custom-* entries (from');
  ...
  for (const [brand, e] of Object.entries(golden.brands)) {
    ...
    L.push(`            motif = ${kq(c.motif ?? 'auto')},`);
    L.push(`            defaultDark = ${c.defaultDark},`);
    L.push(`            defaultRtl = ${c.defaultRtl},`);
    L.push(`            accentOnTertiary = ${c.accentOnTertiary ?? false},`);
    L.push('        ),');
    L.push('    ),');
  }
  L.push(')');
...
// --- the glance subset, for surfaces that cannot import a package -------------------
//
// A Lock Screen Live Activity, a Dynamic Island and an ongoing Android notification are
// drawn by the OPERATING SYSTEM's process from a widget extension or a RemoteViews layout.
// Neither can reach neptune_flutter_ui or neptune_kmp_ui, so before this every native glance
// surface in a Neptune app was styled from imagination (`.orange`, `.green`, `.red`). This is
// the smallest set of resolved roles such a surface needs — status, the neutrals it sits on,
// and the brand primary for "in flight" — emitted as vendorable Swift and Android resources.
// Consumers COPY the generated file and keep the header, so drift is a diff, not a guess.

const OUT_GLANCE_SWIFT = join(
  ROOT,
  'packages/neptune_tokens/generated/native/swift/OdysseyGlanceTokens.swift',
);
const OUT_GLANCE_ANDROID_LIGHT = join(
  ROOT,
  'packages/neptune_tokens/generated/native/android/values/odyssey_glance_tokens.xml',
);
const OUT_GLANCE_ANDROID_DARK = join(
  ROOT,
  'packages/neptune_tokens/generated/native/android/values-night/odyssey_glance_tokens.xml',
);
...
const glanceArgb = (brand, mode, role) =>
  toArgb(oklchToRgb255(...data[brand].colors[mode][`md-sys-color-${role}`]));
const lowerCamel = (role) =>
  role.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
const snake = (role) => role.replace(/-/g, '_');

function emitGlanceSwift() {
  const L = [];
  L.push('// GENERATED by tools/codegen.mjs — DO NOT EDIT.');
  L.push('// Source of truth: packages/neptune_tokens/assets/themes.css');
  L.push('// Regenerate: node tools/codegen.mjs · Verify: node tools/codegen.mjs --check');
  L.push('//');
  L.push('// The glance subset of Neptune Odyssey: the roles a Lock Screen, a Dynamic Island or a');
  L.push('// widget may draw with. Vendored into app targets verbatim - a widget extension cannot');
  L.push('// import the Flutter or KMP packages - so keep this header intact and diff against');
  L.push('// the copy in neptune_odyssey when the system moves.');
  L.push('');
  ...
  L.push('#endif');
  L.push('');
  L.push('/// One role, resolved for both appearances. ARGB, as themes.css resolves it.');
  L.push('public struct OdysseyGlanceRole {');
  L.push('    public let light: UInt32');
  L.push('    public let dark: UInt32');
  L.push('');
  L.push('    public init(light: UInt32, dark: UInt32) {');
  L.push('        self.light = light');
  L.push('        self.dark = dark');
  L.push('    }');
  L.push('');
  L.push('    /// Follows the appearance of the surface it is drawn on.');
  L.push('    public var color: Color {');
  L.push('        #if canImport(UIKit)');
  L.push('        return Color(UIColor { traits in');
  L.push('            traits.userInterfaceStyle == .dark');
  L.push('                ? OdysseyGlanceRole.uiColor(self.dark)');
  L.push('                : OdysseyGlanceRole.uiColor(self.light)');
  L.push('        })');
  L.push('        #else');
  L.push('        return OdysseyGlanceRole.swiftUIColor(light)');
  L.push('        #endif');
  L.push('    }');
  L.push('');
  L.push('    #if canImport(UIKit)');
  L.push('    static func uiColor(_ argb: UInt32) -> UIColor {');
  L.push('        UIColor(');
  L.push('            red: CGFloat((argb >> 16) & 0xFF) / 255,');
  L.push('            green: CGFloat((argb >> 8) & 0xFF) / 255,');
  L.push('            blue: CGFloat(argb & 0xFF) / 255,');
  L.push('            alpha: CGFloat((argb >> 24) & 0xFF) / 255');
  L.push('        )');
  L.push('    }');
  L.push('    #endif');
  L.push('');
  L.push('    static func swiftUIColor(_ argb: UInt32) -> Color {');
  L.push('        Color(');
  L.push('            red: Double((argb >> 16) & 0xFF) / 255,');
  L.push('            green: Double((argb >> 8) & 0xFF) / 255,');
  L.push('            blue: Double(argb & 0xFF) / 255,');
  L.push('            opacity: Double((argb >> 24) & 0xFF) / 255');
  L.push('        )');
  L.push('    }');
  L.push('}');
  L.push('');
  L.push('/// The glance roles of one brand.');
  L.push('public struct OdysseyGlanceBrand {');
  for (const role of GLANCE_ROLES) {
    L.push(`    public let ${lowerCamel(role)}: OdysseyGlanceRole`);
  }
  L.push('}');
  L.push('');
  L.push('/// The four reference brands, pinned. A white-label app that generates its own');
  L.push('/// brandprint at runtime still draws its glance surfaces with these: the status roles');
  L.push('/// are system roles and the neutrals are the same in every brand family.');
  L.push('public enum OdysseyGlance {');
  for (const b of BRANDS) {
    L.push(`    public static let ${b} = OdysseyGlanceBrand(`);
    for (const role of GLANCE_ROLES) {
      L.push(
        `        ${lowerCamel(role)}: OdysseyGlanceRole(light: ${glanceArgb(
          b,
          'light',
          role,
        )}, dark: ${glanceArgb(b, 'dark', role)}),`,
      );
    }
    L.push('    )');
  }
  L.push('');
  L.push('    public static let brands: [String: OdysseyGlanceBrand] = [');
  for (const b of BRANDS) {
    L.push(`        "${b}": ${b},`);
  }
  L.push('    ]');
...
function emitGlanceAndroid(mode) {
  const L = [];
  L.push('<?xml version="1.0" encoding="utf-8"?>');
  L.push('<!--');
  L.push('  GENERATED by tools/codegen.mjs - DO NOT EDIT.');
  L.push('  Source of truth: packages/neptune_tokens/assets/themes.css');
  // No "--" inside an XML comment: aapt rejects the file outright.
  L.push('  Regenerate: node tools/codegen.mjs / Verify: pnpm codegen:check');
  L.push('');
  L.push('  The glance subset of Neptune Odyssey for an ongoing notification, a widget or any other');
  L.push(`  RemoteViews surface. This is the ${mode} appearance; the values-night twin carries dark.`);
  L.push('  Vendored into app res/ verbatim - keep this header and diff against neptune_odyssey.');
  L.push('-->');
  L.push('<resources>');
  for (const b of BRANDS) {
    for (const role of GLANCE_ROLES) {
      const argb = glanceArgb(b, mode, role).replace('0x', '#');
      L.push(`    <color name="odyssey_glance_${b}_${snake(role)}">${argb}</color>`);
    }
  }
  L.push('</resources>');
  return L.join('\n') + '\n';
...
const outputs = [
  ...
  [OUT_GLANCE_SWIFT, emitGlanceSwift()],
  [OUT_GLANCE_ANDROID_LIGHT, emitGlanceAndroid('light')],
  [OUT_GLANCE_ANDROID_DARK, emitGlanceAndroid('dark')],
];
